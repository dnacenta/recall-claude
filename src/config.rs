...
/// Load config from .recall-claude.toml in the base dir.
/// Returns defaults if file doesn't exist or is malformed.
pub fn load(base: &Path) -> Config {
    let config_path = base.join(".recall-claude.toml");
    ...
    // Simple TOML parsing — only handle our known fields
    // We avoid a full toml dependency by parsing manually
...
mod tests {
    ...
    fn load_from_file() {
        let tmp = tempfile::tempdir().unwrap();
        fs::write(
            tmp.path().join(".recall-claude.toml"),
            "[ephemeral]\nmax_entries = 3\n",
        )
        .unwrap();
        let cfg = load(tmp.path());

...
pub fn parse_transcript(path: &str, session_id: &str) -> Result<Conversation, String> {
    ...
    for line in reader.lines() {
        ...
        let entry: JsonlEntry = match serde_json::from_str(&line) {
            Ok(e) => e,
            Err(e) => {
                eprintln!("recall-claude: skipping malformed JSONL line: {e}");
                continue;
            }
        };
...
/// Handles day component for multi-day spans (returns total seconds from epoch-ish).
fn parse_timestamp(ts: &str) -> Option<u64> {
    // Format: 2026-03-05T14:30:00.000Z or 2026-03-05T14:30:00Z
    let t_pos = ts.find('T')?;
    ...
    // Parse time (strip fractional seconds)
    ...
    // Rough seconds (enough for duration calculation)
...
pub fn utc_now() -> String {
    // Use system time to generate ISO 8601 timestamp
...
mod tests {
    ...
    fn parse_transcript_basic() {
        ...
        assert_eq!(conv.assistant_message_count, 2); // two text blocks

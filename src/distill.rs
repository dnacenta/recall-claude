...
pub fn run_with_base(base: &Path) -> Result<(), String> {
    ...
    if !memory_path.exists() {
        return Err("MEMORY.md not found. Run `recall-claude init` first.".to_string());
    }
    ...
    eprintln!("\n{BOLD}recall-claude distill{RESET} — MEMORY.md analysis\n");
...
fn find_near_duplicates(lines: &[&str]) -> Vec<(usize, usize)> {
    ...
    for (i, line) in lines.iter().enumerate() {
        ...
        if let Some(&prev_line) = seen.get(&normalized) {
            duplicates.push((prev_line + 1, i + 1)); // 1-indexed
        } else {
...
fn count_stale_entries(lines: &[&str]) -> usize {
    ...
    let mut stale = 0;
    for line in lines {
        // Look for date patterns YYYY-MM
...
fn find_empty_sections(lines: &[&str]) -> Vec<String> {
    ...
    for line in lines {
        ...
        if is_any_header {
            ...
            // Only track ## sections for emptiness, not top-level # titles
...
mod tests {
    ...
    fn find_sections_basic() {
        ...
        let sections = find_sections(&lines);
        assert_eq!(sections.len(), 3); // Memory, Server, Projects
    }
    ...
    fn analyze_healthy_memory() {
        ...
        let suggestions = analyze_memory(&lines);
        // Under 140 lines, no big sections — should be minimal

...
pub fn run_with_base(base: &Path) -> Result<(), String> {
    if !base.exists() {
        return Err("~/.claude directory not found. Run `recall-claude init` first.".to_string());
    }
    ...
    let overall = if base.join("conversations").exists()
        ...
        && base.join("rules/recall-claude.md").exists()
    {
        format!("{GREEN}healthy{RESET}")
    } else {
        issues.push("Run `recall-claude init` to complete setup".to_string());
        format!("{YELLOW}incomplete{RESET}")
    };

    eprintln!("\n{BOLD}recall-claude{RESET} — {overall}\n");
    ...
    // Protocol
    let protocol_path = base.join("rules/recall-claude.md");
    if protocol_path.exists() {
        eprintln!("  Protocol        {GREEN}installed{RESET}");
    } else {
        eprintln!("  Protocol        {RED}missing{RESET}");
        issues.push("Protocol file missing — run `recall-claude init`".to_string());
    }
    ...
    if settings_path.exists() {
        ...
        let has_session_end = has_hook(&settings, "SessionEnd", "recall-claude");
        if has_session_end {
            eprintln!("  Hooks           {GREEN}SessionEnd active{RESET}");
        } else {
            eprintln!("  Hooks           {RED}SessionEnd missing{RESET}");
            issues.push("SessionEnd hook not configured — run `recall-claude init`".to_string());
        }
    } else {
...
fn find_date_range(dir: &Path) -> (Option<String>, Option<String>) {
    ...
    if let Ok(entries) = fs::read_dir(dir) {
        for entry in entries.flatten() {
            ...
            let name = name.to_string_lossy();
            if name.starts_with("conversation-") && name.ends_with(".md") {
                // Read first few lines to find date in frontmatter
